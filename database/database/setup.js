...
async function setup() {
  ...
  const municipiosArgentina = require('../fixtures/argentina/municipios').municipios
  ...
  // Create five community municipios Argentina
  for (let m of municipiosArgentina) {
    const uuid = await Geopolitic.getUuidByNameAndLevel(m.provincia.nombre, 4)
    await Geopolitic.create({
      name: m.nombre,
      level: 5,
      in_uuid: uuid
    })
    console.log("Municipio: ", m.nombre, " de la Provincia de: ", m.provincia.nombre)
  }
